...
;;;
;;;Discription: this file will lode value 15 in to R1 register 0 into R2 register.
;;;             then it will perform rotate right through carry operation on R1 and update all flags.
;;;             when C flag is 1 then R2 will be incriment by 1 and then if zero flag is one we will end the code and start over.
;;;		else we will keep rotating R1 unless it becomes 0.
...
DMUX:	.equ 100000b ; Use the microcode data field as regfile DIN
	...
	.org 0			;
        ;;; Start of Microcode for Microcode Assignment 1
	START:	INP  \ - \ R1 \ TSA \ UPALL  \ - \ NEXT \ 15 \ DMUX ; ;;;trancefer 15 into R1
		INP  \ - \ R2 \ TSA \  UPALL  \ - \ NEXT \ 0 \ DMUX ; ;;;trancefer 0 into R2	
	L1:	R1  \ -  \ R1 \ RRC \  UPALL  \ - \ NEXT \ - \ - ; ;;;perform RLC on R1 and update all flages
		-  \ -  \ -  \ - \  -  \ INT  \ LNC \ L2  \ - ; ;;;if(C==0)then move to L2 else increment R
       		R2 \ - \ R2   \ INC \  UPC  \ -  \ NEXT \ -   \ - ; ;;;after incrementing R2 move to next instriction
	L2:	-  \ -  \ -  \ - \  -  \ INT  \ LZ \ END  \ - ; ;;; check for zero flag if it is 1 means R1 is 0
	        -  \ -  \ -  \ - \  -  \ INT  \ LAD \ L1  \ - ; ;;; move to L1
	END:	-  \ -  \ -  \ - \  -  \ INT  \ LAD \ START  \ - ; ;;;Move to Start
        ;;; end of Microode  for Microcode Assignment 1
